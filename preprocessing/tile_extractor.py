...
class TileExtractor():
    ...
    def process_image(self, idx, just_image=False, provider=None):
        ...
        try:
            ...
            if not just_image:
                dir_path = os.path.join(dir_path, "train_images")
            img_slide = open_slide(os.path.join(
                dir_path, f"{idx}.tiff"))
            mask_slide = None
            if not just_image:
                mask_slide = open_slide(os.path.join(
                    self.input_dir, "train_label_masks", f"{idx}_mask.tiff"))
    ...
    def save_tiles_from_coordinates(self, slide, mask, coords, saving_dir, provider):
        tiles_zoom = DeepZoomGenerator(
            slide, tile_size=self.tile_size, overlap=0, limit_bounds=True)
        if mask is not None:
            mask_zoom = DeepZoomGenerator(
                mask, tile_size=self.tile_size, overlap=0, limit_bounds=True)
        for c in coords:
            if mask is not None:
                self.save_tile(
                    tiles_zoom, c, os.path.join(saving_dir, "tiles"))
                self.save_tile(mask_zoom, c, os.path.join(
                    saving_dir, "mask"), grayscale=provider)
            else:
                self.save_tile(tiles_zoom, c, saving_dir)

    def save_tile(self, tiles, coord, saving_dir, grayscale=None):
        if not os.path.exists(saving_dir):
            os.mkdir(saving_dir)
        temp_tile = tiles.get_tile(
            len(tiles.level_tiles)-self.inverse_zoom_level, coord)
        ...
        im.save(os.path.join(
            saving_dir, f"{coord[0]}_{coord[1]}.png"), format='PNG', quality=100)
    ...
    def pyramid_tile_extracting(self, slide, down_sampling_factor, avg_threshold, std_threshold):
        ...
        tiles = DeepZoomGenerator(
            slide, tile_size=self.tile_size, overlap=0, limit_bounds=True)
        down_sampled_tiles = DeepZoomGenerator(
            slide, tile_size=self.tile_size/down_sampling_factor, overlap=0, limit_bounds=True)
        ...
        tiles_shape = tiles.level_tiles[len(
            tiles.level_tiles)-self.inverse_zoom_level]
        cols, rows = tiles_shape
        down_tile_level_corresponding_level = down_sampled_tiles.level_tiles.index(
            tiles_shape)

        passed_coords = []
        for col in range(1, cols-1):
            for row in range(1, rows-1):
                temp_tile = down_sampled_tiles.get_tile(
                    down_tile_level_corresponding_level, (col, row))
